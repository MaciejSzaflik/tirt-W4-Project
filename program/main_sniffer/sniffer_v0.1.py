...
def saveToFile(imgarray, id, object):
    try:
        im = Image.open(StringIO(imgarray))
        #print('type ' + str(id), im.format, im.size, im.mode)
    except IOError:
        return
    ...
    #im.save('temp/aaimg' + str(id) + '.jpg', format=im.format)
...
#               self.TextTcp.delete(1.0,END)
            ...
            if (len(data)) > 0 :#and nextNr < 20 :
                #ffile = open('temp/jpg' + str(nextNr) + '.jpg', 'w')
                #ffile.write(data)
                #ffile.close()
                #ffile = open('temp/raw' + str(nextNr) + '.raw', 'w')
                #ffile.write(packet)
                #ffile.close()
                #FileTCP.write(packet)
                saveToFile(data[37:], nextNr, object)
...
def packetService(packet, next_nr, object):
...
class MyApp(object):
    ...
    def __init__(self, parent):
        ...
        #self.FileTCP = open('temp/tcp.txt', 'w')
        #self.FileUDP = open('temp/udp.txt', 'w')
    ...
    def readPacketFromSocket(self,obj):
        ...
        #start_new_thread(packetService,(packet, self.nextPCK, self))
        packetService(packet, self.nextPCK, self)
...
#               self.TextTcp.delete(1.0,END)
                ...
                if (len(data)) > 0 and self.nextTCP < 20 :
                    ...
                    ffile = open('temp/jpg' + str(self.nextTCP) + '.jpg', 'w')
                    ffile.write(data)
                    ffile.close()
                    ffile = open('temp/raw' + str(self.nextTCP) + '.raw', 'w')
                    ffile.write(packet)
                    ffile.close()
